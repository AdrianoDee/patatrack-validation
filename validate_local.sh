...
#Pixel nTuplets (>3) Selector
from CommonTools.RecoAlgos.recoTrackRefSelector_cfi import recoTrackRefSelector as recoTrackRefSelector_

if "pixelTracks" in process.__dict__:
  process.pixelNTupletTracks = recoTrackRefSelector_.clone(
     minPixelHit = cms.int32(4),
     src = cms.InputTag("pixelTracks"),
     vertexTag = cms.InputTag(''),
     quality = cms.vstring('any')
  )
  process.pixelNTupletTracksPt09 = recoTrackRefSelector_.clone(
     minPixelHit = cms.int32(4),
     src = cms.InputTag("pixelTracks"),
     vertexTag = cms.InputTag(''),
  ...
  process.globalPrevalidationPixelTrackingOnly.insert(0,process.pixelNTupletTracks)
  process.globalPrevalidationPixelTrackingOnly.insert(0,process.pixelNTupletTracksPt09)

  validators = ["trackValidatorPixelTrackingOnly","trackValidatorFromPVPixelTrackingOnly","trackValidatorFromPVAllTPPixelTrackingOnly","trackValidatorBHadronPixelTrackingOnly"]
  validators = [ v for v in validators if v in process.__dict__]
  for val in validators:
    getattr(process,val).label.extend(["pixelNTupletTracks","pixelNTupletTracksPt09"])
